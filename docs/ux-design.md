# MiaoGram 0.1 设计说明 / Design notes

**中文** | [English](#english)

MiaoGram 0.1 的目标很简单：

> 保留 Telegram 的熟悉体验，然后给每天泡在 Telegram 里的用户加几只锋利但不吓人的小猫爪。

这不是未来所有功能的承诺清单，而是当前公开产品方向的快照。

## 产品气质

MiaoGram 应该感觉像 Telegram，而不是一款披着 Telegram 外衣的陌生应用。

- 尽量保留上游导航和视觉语言。
- 把 MiaoGram 增强功能放在独立设置区。
- 优先做可逆的小开关，而不是突然改变全局行为。
- 让高级功能可发现，但不要吵。
- 敏感或社区向功能默认关闭。
...
为什么重要：官方 Telegram 对少量账号很好用；当账号列表开始像小型办公楼时，MiaoGram 想变得更顺手。

### 2. 阅读和频道管理

很多 Telegram 重度用户会关注大量频道。MiaoGram 加入本地组织工具：

- 频道阅读器。
- 手动阅读优先级：必读、补读、归档、其他。
- 不依赖后端服务的本地分组。

目标不是替代文件夹，而是给高频阅读用户一个更安静的补读入口。

### 3. 翻译作为基础预期

许多第三方 Telegram 客户端都包含消息翻译。MiaoGram 0.1 把它视为基础体验：
...
翻译会向所选翻译源发起网络请求，当前来源会在设置里显示。

### 4. 小但高频的体验优化

MiaoGram 包含一组让日常使用更顺手的小开关：

- 隐藏 Stories 或「所有聊天」tab 等视觉干扰。
- 清理外链追踪参数。
- 显示用户 ID 和头像 DC 等技术信息。
- 消息时间可选显示到秒。
- 危险确认对话框使用更安全的默认值。
- 关键词通知静默。

这些不一定是标题级功能，但会在用一两天后留下小猫脚印。
